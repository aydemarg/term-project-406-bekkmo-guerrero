...
Starting from the merged Olympic–World Bank dataset, we removed any variable with more than 50% missing values, variables that would leak the outcome (`year`, `gold`, `silver`, `bronze`), and the raw `total_medals` column once the log target was created. We kept `season` (Summer/Winter) as a predictor because the two Games reward different sports and country strengths. `country` is retained in the data frame so they can be used as a grouping variable for cross-validation, but they are marked as an ID column in the recipe so they do not enter the model as a predictor.
...
Linear Regression produces a much narrower spread of predictions, clustered in a tight band around the training-set mean. The model rarely predicts very low or very high medal counts, which is why its CV RMSE is only marginally better than the baseline. The under-prediction of high-medal countries, due to many observations being clustered about zero on the x-axis, is even more severe than for both Lasso Regression and Random Forest (displayed below).

#### Lasso Regression

![](../output/figures/lasso_pred_vs_actual_medals.png)
Lasso falls between the two: its predictions span a wider range than Linear Regression but still narrower than Random Forest, still over-predicting many observations. The improvement over Linear Regression comes from dropping correlated and uninformative predictors, which stabilizes the remaining coefficients. However, this model still lacks capturing non-linearity and interactions.

#### Random Forest

![](../output/figures/fig17_pred_vs_actual_medals.png)
Random Forest spreads predictions over the widest range of the three models, but still systematically under-predicts the highest-medal countries; actual medal counts of 100+ are predicted at 25–45. This compression at the top is expected for any model trained on a long-tailed target with grouped CV: the held-out test countries include cases that are structurally unlike anything in the training set.
